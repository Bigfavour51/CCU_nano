...
public:
  LGFX(void)
  {
    ...
    { // Set the display panel control.//表示パネル制御の設定を行います。
      ...
      cfg.offset_rotation  =     1;  // hb set from 0 t0 1 Offset of values in the direction of rotation 0 ~ 7 (4 ~ 7 are upside down) // 回転方向の値のオフセット 0~7 (4~7は上下反転)
    ...
    { // Set the touch screen control. (Delete if not needed) // タッチスクリーン制御の設定を行います。（必要なければ削除）
      ...
      cfg.x_min      = 200;    // Minimum X value (raw value) obtained from touch screen // タッチスクリーンから得られる最小のX値(生の値)
      cfg.x_max      = 3700;  // Maximum X value (raw value) obtained from the touch screen // タッチスクリーンから得られる最大のX値(生の値)
      cfg.y_min      = 240;    // Minimum Y value (raw value) obtained from touch screen // タッチスクリーンから得られる最小のY値(生の値)
      cfg.y_max      = 3800;  // Maximum Y value (raw value) obtained from the touch screen // タッチスクリーンから得られる最大のY値(生の値)
      ...
      cfg.offset_rotation =6;// hb set from 0 t0 6 Adjustment when the display and touch orientation do not match Set with a value from 0 to 7 // 表示とタッチの向きのが一致しない場合の調整 0~7の値で設定
